...
module Algorithms::Rank::RSVD
  ...
  def svd(question_id, start_k = 0, end_k = 10, h = 0.001)
    if (ret = stored_solution(question_id)) && ret.is_a?(Array)
      ret
    else
      num_prompts = ret || ::Prompt.count(:conditions => { :question_id => question_id })
      votes, visits, items = make_vote_data(question_id)
      v_len = visits.length
      p_len = items.length
      h, v_old, p_old = load_solution(question_id) || [
        h,
...
#          votes[visit] << [winner, loser]
    ...
    def stored_solution(question_id) #:doc:
      ...
      if File.exists?(name) && Time.now - File.mtime(name) < 1.day
        ...
        num_prompts > sol[0] * 1.05 ? num_prompts : sol[1..sol.length]
      end
    end
end